    ...
    pub fn gen_multi_chip(
        ...
        let (res1, (res2, res3)) = utils::maybe_par_join(
            || {
                ...
                render_with_fmt(&self.tera, "chips.tera", &ctx, format, out)
            },
            || {
                utils::maybe_par_join(
                    || {
                        utils::into_maybe_par_iter(&multi.chips).try_for_each(
                            |chip| -> io::Result<()> {
                                ...
                                self.gen_chip(chip, Some(".."), Utils::Super, format, out)
                            },
                        )
                    },
                    || {
